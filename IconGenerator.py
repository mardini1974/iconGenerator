...
def generate_icon_images(icon_filename):
    sizes = [16, 20, 29, 32, 40, 60, 50, 57, 58, 72, 76, 80, 87, 100, 114, 120, 144, 152, 167, 180, 1024]
...
def generate_fusion360_icon_images(icon_filename):
    ...
    for size in sizes:
        ...
        try:
            size1 = size0[1].split("@")[0]
            multiplier = 2
        except:
            size1 = size0[1]
            multiplier =1

        i = img.resize((multiplier * int(size0[0]), multiplier * int(size1)), Image.ANTIALIAS)
        img_filename = f"{size}-normal.png"
        i.save(img_filename, format="PNG")
        color_image = Image.open(f"{img_filename}").convert('LA').save(f"{size}-disabled.png", format="PNG")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--fusion",
                        help="Generate fusion360 icons assets")
    parser.parse_args()
    ...
    if args.fusion:
        print ("Generating icons with image {0}".format(args.fusion))
        generate_fusion360_icon_images(args.fusion)